import os
import datetime
import customtkinter as ctk
from LatteArtLogic import LatteArt
from PIL import Image, ImageEnhance
from tkinter import filedialog, messagebox

# Default Settings Param
default_bright = 1.8
default_contrast = 2.4
default_offset = 20
default_zoom = 1.0

class LatteArtUI(ctk.CTk):
    def __init__(self, logic_engine):
        super().__init__()
        self.logic = logic_engine

        self.title("Espresso Tycoon Custom Latte Art Tool")
        self.geometry("1150x850")

        self.raw_image = None
        self.processed_image = None
        self.perc_labels = {}

        # Movement and Dragging states
        self.off_x = 0
        self.off_y = 0
        self.mouse_start_x = 0
        self.mouse_start_y = 0

        self.grid_columnconfigure(1, weight=1)
        self.grid_rowconfigure(0, weight=1)

        self.setup_sidebar()
        self.setup_previews()
        self.protocol("WM_DELETE_WINDOW", self.on_closing)

    def on_closing(self):
        """Clean up resources before exiting."""            # Clear image references to free memory immediately
        self.raw_image = None
        self.processed_image = None

        # Destroy the UI
        self.destroy()
        # Ensure the script exits
        self.quit()

    def setup_sidebar(self):
        self.sidebar = ctk.CTkFrame(self, width=280, corner_radius=0)
        self.sidebar.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)

        ctk.CTkLabel(self.sidebar, text="Latte Art Editor", font=("Arial", 22, "bold")).pack(pady=20)

        # Sliders
        self.bright_slider = self.create_slider("Brightness", 0.5, 3.0, default_bright)
        self.contrast_slider = self.create_slider("Contrast", 0.5, 3.0, default_contrast)
        self.offset_slider = self.create_slider("Curves Offset", 0, 150, default_offset)
        self.zoom_slider = self.create_slider("Zoom / Scale", 1.0, 0.05, default_zoom)

        # Controls Hint
        hint_txt = "💡 Drag image to move\n💡 Mouse wheel to zoom"
        ctk.CTkLabel(self.sidebar, text=hint_txt, font=("Arial", 11), text_color="gray60", justify="left").pack(pady=10)

        self.coffee_bg_var = ctk.BooleanVar(value=False)
        self.coffee_switch = ctk.CTkSwitch(self.sidebar, text="Preview on Coffee", variable=self.coffee_bg_var,
                                           command=self.update_preview)
        self.coffee_switch.pack(pady=15)

        ctk.CTkButton(self.sidebar, text="Reset Defaults", fg_color="gray40", command=self.reset_settings).pack(pady=10)
        ctk.CTkFrame(self.sidebar, height=2, fg_color="gray30").pack(fill="x", padx=20, pady=20)

        ctk.CTkLabel(self.sidebar, text="Select Save Slot:", font=("Arial", 14, "bold")).pack(pady=5)
        self.slot_dropdown = ctk.CTkOptionMenu(self.sidebar, values=list(self.logic.save_slots.keys()))
        self.slot_dropdown.set("Auto Save")
        self.slot_dropdown.pack(pady=10, padx=20, fill="x")

        ctk.CTkButton(self.sidebar, text="- Select Image -", command=self.load_image).pack(pady=10, padx=20, fill="x")
        self.send_btn = ctk.CTkButton(self.sidebar, text="- Send to Game -", state="disabled", fg_color="#2ecc71",
                                      hover_color="#27ae60", command=self.confirm_and_save)
        self.send_btn.pack(pady=10, padx=20, fill="x")

        ctk.CTkButton(self.sidebar, text="📁 Open Save Folder", fg_color="transparent", border_width=1,
                      command=self.handle_open_folder).pack(pady=20, padx=20, fill="x")

    def setup_previews(self):
        self.preview_frame = ctk.CTkFrame(self)
        self.preview_frame.grid(row=0, column=1, padx=20, pady=20, sticky="nsew")
        self.preview_frame.grid_columnconfigure((0, 1), weight=1)
        self.preview_frame.grid_rowconfigure(1, weight=1)

        ctk.CTkLabel(self.preview_frame, text="Original Source").grid(row=0, column=0, pady=10)
        ctk.CTkLabel(self.preview_frame, text="Processed Output (512x512)").grid(row=0, column=1, pady=10)

        self.original_label = ctk.CTkLabel(self.preview_frame, text="No image", fg_color="gray20", corner_radius=10)
        self.original_label.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")

        self.processed_label = ctk.CTkLabel(self.preview_frame, text="No image", fg_color="gray20", corner_radius=10)
        self.processed_label.grid(row=1, column=1, padx=10, pady=10, sticky="nsew")

        # Bind Mouse Interactions
        self.processed_label.bind("<Button-1>", self.start_drag)
        self.processed_label.bind("<B1-Motion>", self.do_drag)
        self.bind("<MouseWheel>", self.do_scroll_zoom)  # Windows
        self.bind("<Button-4>", self.do_scroll_zoom)  # Linux Scroll Up
        self.bind("<Button-5>", self.do_scroll_zoom)  # Linux Scroll Down

    def create_slider(self, label, min_val, max_val, start_val):
        header = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        header.pack(fill="x", padx=20)
        ctk.CTkLabel(header, text=label).pack(side="left")

        perc = ctk.CTkLabel(header, text="0%", font=("Arial", 10, "italic"), text_color="#ffddb2")
        perc.pack(side="right")
        self.perc_labels[label] = perc

        slider = ctk.CTkSlider(self.sidebar, from_=min_val, to=max_val,
                               command=lambda v, l=label: self.on_slider_move(v, l))
        slider.set(start_val)
        slider.pack(pady=(0, 15), padx=20, fill="x")
        self.update_perc_text(label, start_val, min_val, max_val)
        return slider

    def on_slider_move(self, value, label):
        ranges = {"Brightness": (0.5, 3.0), "Contrast": (0.5, 3.0), "Curves Offset": (0, 150),
                  "Zoom / Scale": (1.0, 0.05)}
        m, x = ranges[label]
        self.update_perc_text(label, value, m, x)
        self.update_preview()

    def update_perc_text(self, label, value, min_v, max_v):
        if label == "Zoom / Scale":
            percentage = int(value * 100)
        else:
            percentage = int(((value - min_v) / (max_v - min_v)) * 100)
        self.perc_labels[label].configure(text=f"{max(0, percentage)}%")

    # --- Mouse Interaction Logic ---
    def start_drag(self, event):
        self.mouse_start_x = event.x
        self.mouse_start_y = event.y

    def do_drag(self, event):
        if self.raw_image:
            dx = (event.x - self.mouse_start_x) / 400.0
            dy = (event.y - self.mouse_start_y) / 400.0
            self.off_x -= dx
            self.off_y -= dy
            self.mouse_start_x = event.x
            self.mouse_start_y = event.y
            self.update_preview()

    def do_scroll_zoom(self, event):
        if not self.raw_image: return
        # Windows uses event.delta, Linux uses Button 4/5
        delta = event.delta if hasattr(event, 'delta') else (120 if event.num == 4 else -120)
        current_zoom = self.zoom_slider.get()
        step = 0.05 if delta > 0 else -0.05
        new_zoom = max(0.05, min(1.0, current_zoom - step))
        self.zoom_slider.set(new_zoom)
        self.on_slider_move(new_zoom, "Zoom / Scale")

    def update_preview(self, _=None):
        if self.raw_image:
            self.processed_image = self.logic.process_image(
                self.raw_image, self.bright_slider.get(), self.contrast_slider.get(),
                self.offset_slider.get(), self.zoom_slider.get(), self.off_x, self.off_y
            )

            display_img = self.processed_image.copy()
            if self.coffee_bg_var.get():
                coffee_rgb = (178, 104, 83)
                milk_base_rgb = (255, 221, 178)
                background = Image.new("RGBA", display_img.size, coffee_rgb + (255,))
                luma_mask = display_img.convert("L")
                luma_mask = luma_mask.point(lambda p: 0 if p < 180 else p)
                luma_mask = ImageEnhance.Contrast(luma_mask).enhance(1.3)
                tint = 0.85
                final_color = tuple(int(milk_base_rgb[i] * tint + coffee_rgb[i] * (1 - tint)) for i in range(3))
                milk_layer = Image.new("RGBA", display_img.size, final_color + (255,))
                background.paste(milk_layer, (0, 0), luma_mask)
                display_img = background

            self.processed_label.configure(image=ctk.CTkImage(display_img, size=(400, 400)), text="")

    def reset_settings(self):
        self.off_x, self.off_y = 0, 0
        self.bright_slider.set(default_bright)
        self.contrast_slider.set(default_contrast)
        self.offset_slider.set(default_offset)
        self.zoom_slider.set(default_zoom)
        for l, r in {"Brightness": (0.5, 3.0), "Contrast": (0.5, 3.0), "Curves Offset": (0, 150),
                     "Zoom / Scale": (1.0, 0.05)}.items():
            val = default_bright if l == "Brightness" else default_contrast if l == "Contrast" else default_offset if l == "Curves Offset" else default_zoom
            self.update_perc_text(l, val, r[0], r[1])
        self.update_preview()

    def load_image(self):
        path = filedialog.askopenfilename(filetypes=[("Images", "*.png;*.jpg;*.jpeg;*.bmp")])
        if path:
            self.raw_image = Image.open(path)
            orig_p = self.raw_image.copy()
            orig_p.thumbnail((400, 400))
            self.original_label.configure(image=ctk.CTkImage(orig_p, size=orig_p.size), text="")
            self.update_preview()
            self.send_btn.configure(state="normal")

    def handle_open_folder(self):
        path = self.logic.get_save_path(self.slot_dropdown.get())
        self.logic.open_folder(path)

    def confirm_and_save(self):
        if not self.processed_image: return
        target_dir = self.logic.get_save_path(self.slot_dropdown.get())
        try:
            os.makedirs(target_dir, exist_ok=True)
            if messagebox.askyesno("Confirm", f"Send to {self.slot_dropdown.get()}?"):
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"Art_{timestamp}.png"
                self.processed_image.save(os.path.join(target_dir, filename), "PNG")
                messagebox.showinfo("Done", f"Latte Art Sent!\nSaved as: {filename}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save: {e}")

if __name__ == "__main__":
    app = LatteArtUI(LatteArt())
    app.mainloop()